...
a = load('feat_opt/CPI/rand_4fold_10it_2964N_111D_trial1.mat');
b = load('feat_opt/CPI/rand_4fold_10it_2964N_111D_trial2.mat');
c = load('feat_opt/CPI/rand_4fold_10it_more_samples.mat');
n=50;
xx_a = 1:1:numel(a.mean_BER_Te);
xx_b = 1:1:numel(b.mean_BER_Te);
xx_c = 1:1:numel(c.mean_BER_Te);
myfig = figure('Visible','on','units','pixels','position',[800 600 figw figh]);
hold on; grid on; box on;
plot([25 25],[0.5 1],'k--','color',[150,150,150]/255);
p1 = plot(xx_a,a.mean_kappa_Te,'color',type_color,'linewidth',1.5);
addaxislabel(1,'HSS');
addaxis(xx_a,a.mean_BER_Te,[0 0.5],'--','color',type_color,'linewidth',1.5);
addaxislabel(2,'BER');
addaxis(xx_b,b.mean_BER_Te,[0 0.5],'--','color',rim_color,'linewidth',1.5);
addaxis(xx_c,c.mean_BER_Te,[0 0.5],'--','color',melt_color,'linewidth',1.5);
p2 = plot(xx_b,b.mean_kappa_Te,'color',rim_color,'linewidth',1.5);
p3 = plot(xx_c,c.mean_kappa_Te,'color',melt_color,'linewidth',1.5);
axis([0 50 0.5 1]);
legend([p1 p2 p3],{'new_trial1','new_trial2','old'});
box on;
set(gca,'Fontsize',16);
