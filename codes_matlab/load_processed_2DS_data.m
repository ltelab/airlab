...
%             X(i,j) = roi.H.Homogeneity; Xlab{j} = 'Haralick homogeneity'; j=j+1; %56
            ...
            % Fractal
            X(i,j) = roi.F; Xlab{j} = 'boxcounting fractal dim'; j=j+1; %32
            X(i,j) = roi.F_jac; Xlab{j} = 'theoretical fractal dim'; j=j+1; %33
            ...
            X(i,j) = roi.roundness; Xlab{j} = 'roundness'; j=j+1; %37
            X(i,j) = roi.compactness; Xlab{j} = 'compactness'; j=j+1; %38
            if roi.E.a >= roi.E.b
                X(i,j) = roi.E.b/roi.E.a; Xlab{j} = 'aspec ratio'; j=j+1; %39
            ...
            X(i,j) = roi.nb_holes; Xlab{j} = 'N holes'; j=j+1; %42
            ...
            X(i,j) = (2*pi*get_struct_field(roi.C_out,'r'))/roi.perim; Xlab{j} = 'circle out perim ratio'; j=j+1; %47
...
%             X(i,j) = roi.local_std; Xlab{j} = 'std_intens'; j=j+1; %79
            X(i,j) = roi.complex; Xlab{j} = 'complex'; j=j+1; %48
            ...
            X(i,j) = roi.touch_edge; Xlab{j} = 'is touching image edge'; j=j+1; %66
            X(i,j) = roi.touch_edge_ratio; Xlab{j} = 'ratio of perimeter touching the edge'; j=j+1; %67
            X(i,j) = roi.n_true; Xlab{j} = 'total number of 1 pixels'; j=j+1; %68
            X(i,j) = roi.ntrue_ratio; Xlab{j} = 'ratio 1 pixels within mask / total 1 pixels'; j=j+1; % 69
