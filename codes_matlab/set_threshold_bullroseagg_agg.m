...
columns_dir = '../training_set/subclassification/bulrosagg_vs_others/bulrosagg';
needles_dir = '../training_set/subclassification/bulrosagg_vs_others/otheragg';
...
for i=1:numel(columns_list)
    ...
    columns(i,1) = roi.F_jac; % rect AR
    columns(i,2) = roi.icpca.b.jana_kov(1); % rect ecc.
    columns(i,3) = roi.icpca.b.jana_kov(2);            % D90 AR
    columns(i,4) = roi.frame_fraction;       % ellipse AR
...
for i=1:numel(needles_list)
    ...
    needles(i,1) = roi.F_jac; % rect AR
    needles(i,2) = roi.icpca.b.jana_kov(1); % rect ecc.
    needles(i,3) = roi.icpca.b.jana_kov(2);            % D90 AR
    needles(i,4) = roi.frame_fraction;       % ellipse AR
...
figure;
scatterhist(xx,yy,'Group',type,'linewidth',[2,2]);%,'displaystyle','bars');
xlabel('Fractal dimension [a.u.]');
ylabel('cov(s(30°),0°) [a.u.]');
legend('BR aggregates','other aggregates');
set(gca,'fontsize',12);
